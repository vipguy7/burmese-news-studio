  ...
  async function runTranslate() {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error(
          d.error || (r.status === 429 ? "Rate limit reached." : "Generation failed"),
        );
      }
      setResult(d);
    } catch (e) {
        ...
        <aside className="lg:col-span-2 space-y-4">
          ...
          {showAdd && (
            <div className="border border-border bg-card p-4 space-y-3">
              ...
              {saveError && <div className="text-xs text-destructive font-sans">{saveError}</div>}
              <button
                onClick={saveItem}
                disabled={saving}
                    ...
                    {result.sources.map((s) => (
                      <li key={s.id} className="flex items-center gap-2">
                        <Sparkles
                          className={cn(
                            "w-3 h-3",
                            s.selected ? "text-primary" : "text-muted-foreground/50",
                          )}
                        />
                        <span>{s.title}</span>
